...
async function shouldSearch(message, memoryContext, currentDateTime) {
  // Only consider searching if message ends with ? or contains explicit search signals
  const trimmed = message.trim();
  const endsWithQuestion = trimmed.endsWith('?');
  const hasExplicitSignal = [
    'search for', 'look up', 'find out', 'google', 'search the web',
    'what is the latest', 'any news', 'have you heard', 'current events',
  ].some(function(s) { return trimmed.toLowerCase().includes(s); });

  if (!endsWithQuestion && !hasExplicitSignal) return false;
  if (!BRAVE_KEY) return false;

  // Ask Claude if it can answer confidently without searching
  try {
    const checkPrompt = [
      'Current date and time: ' + currentDateTime,
      '',
      'Memory context about the user:',
      memoryContext || '(none)',
      '',
      'The user asked: "' + message + '"',
      '',
      'Can you answer this question accurately and completely from your training',
      'knowledge and the memory context above, WITHOUT needing to search the web?',
      '',
      'Answer only YES or NO.',
      'Answer NO if the question requires: current events, live data, recent news,',
      'real-time prices, todays weather, recent releases, or anything that changes',
      'frequently and where being out of date would meaningfully hurt the answer.',
      'Answer YES if you can give a genuinely useful complete answer from what you know.',
    ].join('\n');

...
import { scoreEntry } from '../memory/scorer.js';
...
async function loadMemory(userId) {
  try {
    // Fetch more than we need, then prioritize using the shared Constitution
    // scoring function (memory/scorer.js) -- the same one memories.js uses
    // for the panel, so injection and panel display are no longer two
    // independently-drifting scoring systems.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Anonymous chat access is closed. Safety check above still ran for this
    // message regardless of auth status -- crisis response is never gated
    // behind sign-in. But ordinary use requires an account.
    if (!isLoggedIn) {
      return res.status(200).json({
        type: 'message',
        message: 'Hi -- I\'m Thais, and I\'m glad you\'re here. To actually talk with me, you\'ll need to sign in or create a free account first. It only takes a minute, and once you\'re signed in I can actually remember our conversations and be useful to you over time.',
        workflow: WORKFLOWS.general,
        memoryPrompt: null,
        searchPerformed: false,
        searchQuery: '',
